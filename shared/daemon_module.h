 ...
 *     (교체 직후 실측 raw roll=4.26 pitch=3.74). 장착 오프셋을 다시 재야 한다.
 *
 ...
 *
 *  ⚠️ IMU 마운트가 한쪽만 지지되어 있어(브래킷 다리 1개) 장착각이 진동·자중
 *    으로 움직인다. 이 상수는 마운트가 안 변한다는 전제 위에 서 있으므로,
 *    다리를 채우기 전에는 값이 오래 유지되지 않는다. 게이트가 갑자기 막거나
 *    갑자기 통과하기 시작하면 리그가 아니라 이쪽을 먼저 의심할 것.
 *
 *  0 으로 두면 오프셋 없이 중력벡터 각 그대로 판정한다(종전 동작). */
#define IMU_INSTALL_ROLL_DEG    4.26f
#define IMU_INSTALL_PITCH_DEG   3.74f
